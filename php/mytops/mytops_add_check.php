...
try{
  ...
  if($picture_data['size'] > 0){
      ...
      if($picture_data['size'] > 1000000){
          ...
          move_uploaded_file($picture_data['tmp_name'],'../../images/mytops/'.$picture_data['name']);
          print('<img id="picture-check" src="../../images/mytops/'.$picture_data['name'].'">');
          print('<br/>');
  ...
  if(empty($_POST['name']) || $picture_data['size'] > 1000000){
      ...
      print('<form method="post" action="mytops_add_done.php">');
